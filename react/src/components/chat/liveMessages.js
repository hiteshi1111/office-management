...
const LiveMessages = ({ 
    ...
    function handleEmojiClick(emoji) {
        PutRequest(`${process.env.REACT_APP_URL}/message/reaction/${item?._id}`, {
            ...
            ChatSocket.emitEmojiReactEvent(item?._id, emoji)
            dispatch(chatActions.setSelectedMessage(""))
        }).catch((error) => {
            console.log("emoji react error", error);
        });
    }
    ...
    const showAvatar = item?.from !== accountInfo?.data._id && (i === 0 || isDifferentSender(item, allMessages[i - 1]?.data) || !isSameTime(item, allMessages[i - 1]?.data));
    ...
    return (
        ...
        >
            ...
            <div className='msg-chat-order relative max-w-[60%] sm:max-w-[50%]'>
                {(i === 0 || isDifferentSender(item, allMessages[i - 1]?.data) || !isSameTime(item, allMessages[i - 1]?.data)) && (
                    <div ref={messageRef} className={`text-[#aaa] text-[12px] pt-[10px] flex gap-[5px] ${item.from === accountInfo?.data._id ? "text-right flex-row-reverse" : "text-left flex-row"}`}>
                ...
                <div className={`relative break-all ${item.from === accountInfo?.data._id ? "rounded-l-[20px] rounded-t-[20px]" : "rounded-r-[20px] rounded-t-[20px]"} ${item.from !== accountInfo?.data._id && !showAvatar && "!rounded-l-none rounded-ee-[20px] !rounded-bl-[20px]"} ${!item?.isRemoved && (isEmojiMessage(message) && message.length <= 6) ? "border-0 bg-transparent" : "border bg-white shadow-md p-[10px] px-[20px]"}`}>
                    {item.isRemoved ? (
                        ...
                        (item.forwardedFrom || item.replyTo) ? (
                            <div className='bg-white'>
                                ...
                                <div
                                    className={`${isEmojiMessage(message) ? "text-[30px]" : "text-[14px]"}`}
                    ...
                    {!item.isRemoved && currentChat?.blockedBy?.length === 0 && currentChat?.isFriend && (
                        <div className={`absolute top-[5px] ${item.from === accountInfo?.data._id ? "left-[-20px]" : "right-[-20px]"}`}>
                            {item.from !== accountInfo?.data._id && (
                    ...
                    {message && !item.isRemoved && item.reactions?.length > 0 && (
                        <div className='mb-[-25px] flex flex-wrap gap-[5px] pb-[10px]'>
                            {item.reactions.map((item, i) => (
                                <div key={i}
                                    ...
                                    className='text-[18px] h-[30px] w-[30px] bg-white rounded-full flex justify-center items-center shadow-md cursor-pointer'
                                >
                                    {item.emoji}
                                    {hoveredReaction === i && (
                                        <div className='absolute flex flex-col top-[-35px] w-[80px] h-[50px] text-[12px] p-[8px] bg-[#111827] text-white rounded shadow-md items-center justify-center'>
            ...
            <div className={`msg-avatar-order ${item?.from !== accountInfo?.data._id ? "block" : "hidden"}`}>
                <Avatar
                    src={!currentChat?.blockedBy?.includes(currentChat?.userData._id)
            ...
            <div className={`mb-[24px] relative ${item.isRemoved ? "opacity-0" : "group-hover:opacity-100 opacity-0"}`}>
                ...
                <MessageActions
                    data={item}
                    className={`${item._id === selectedAction ? "block" : "hidden"} ${i + 1 === allMessages?.data?.length ? "bottom-[20px]" : "top-[20px]"} ${item.from === accountInfo?.data._id ? "right-0" : "left-0"}`}
                    message={message}
