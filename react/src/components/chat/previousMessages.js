        ...
        >
            <div className='msg-chat-order relative max-w-[80%] sm:max-w-[50%]' >
                {(i === 0 || isDifferentSender(item, allMessages[i - 1]) || !isSameTime(item, allMessages[i - 1])) && (
                    <div ref={messageRef} id={item?.createdAt} className={`text-[#aaa] text-[12px] flex pt-[10px] gap-[5px] ${item?.from === accountInfo?.data?._id ? "text-right flex-row-reverse" : "text-left flex-row"}`} >  
